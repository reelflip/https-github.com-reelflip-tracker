        ...
        <div className="p-6 border-b border-slate-800 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1 flex items-center">
                {currentUser?.role}
                {currentUser?.role === 'ADMIN' && <span className="ml-1 opacity-75">• v5.0</span>}
            </p>
          </div>
        </div>
      ...
      <main className="flex-1 min-w-0 flex flex-col h-screen overflow-hidden relative">
        ...
        <header className="md:hidden bg-slate-900 text-white px-4 py-3 flex justify-between items-center sticky top-0 z-30 shadow-md">
             ...
             <div className="flex items-center space-x-3">
                 {currentUser?.role === 'ADMIN' && <span className="text-[10px] bg-slate-800 px-2 py-1 rounded text-slate-400">v5.0</span>}
                 {/* User Avatar Tiny */}
        ...
        {mobileMenuOpen && (
            <div className="md:hidden fixed inset-0 z-40 bg-slate-900/95 backdrop-blur-md pt-20 px-4 pb-28 overflow-y-auto animate-in fade-in slide-in-from-bottom-10 duration-200">
                ...
                <div className="mt-8 text-center">
                    <p className="text-white/20 text-[10px]">IITGEEPrep Mobile v5.0</p>
                </div>
            </div>
        )}
